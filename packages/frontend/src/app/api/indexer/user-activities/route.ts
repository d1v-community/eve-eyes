...
} from '~~/server/indexer/user-activity-repository.mjs'
...
function isBadRequest(error: unknown) {
  return (
    ...
    (error.message.includes('must be a positive integer') ||
      ...
      error.message.includes('must be a valid timestamp') ||
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listUserActivities(sql, {
      filters: parseUserActivityFilters(url.searchParams),
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to list user activities'
    let status = 500
