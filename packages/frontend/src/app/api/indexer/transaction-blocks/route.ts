...
export async function GET(request: Request) {
  try {
    ...
    const result = await listTransactionBlocks(sql, {
      filters: parseTransactionBlockFilters(url.searchParams),
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to list transaction blocks'
    let status = 500
