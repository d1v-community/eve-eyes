...
export default function AtlasSystemDetails({
  ...
  return (
    <div className="rounded-[1.6rem] border border-stone-300/70 bg-[linear-gradient(180deg,rgba(255,252,246,0.88),rgba(247,242,234,0.72))] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)] dark:border-slate-800 dark:bg-slate-900/60">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-base font-medium text-stone-950 dark:text-slate-100">
      ...
      {system ? (
        <div className="space-y-4 text-sm text-stone-600 dark:text-slate-300">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-stone-300/70 bg-white/80 p-3 dark:border-slate-800 dark:bg-slate-950/50">
          ...
          <div className="grid gap-2">
            {system.gateLinks.length > 0 ? (
              system.gateLinks.map((gate) => (
                <div
                  key={gate.id}
                  className="rounded-2xl border border-stone-300/70 bg-white/80 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50"
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-stone-300/80 bg-stone-50/80 px-2.5 py-1 text-[10px] uppercase tracking-[0.28em] text-stone-500 transition hover:border-amber-300 hover:text-amber-700 dark:border-slate-700 dark:text-slate-300 dark:hover:border-sky-700"
                    >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-emerald-200/80 bg-emerald-50/80 px-3 py-1 text-[10px] uppercase tracking-[0.28em] text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-900/70 dark:bg-emerald-950/30 dark:text-emerald-300"
                    >
                      Origin
                    </button>
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-amber-200/80 bg-amber-50/80 px-3 py-1 text-[10px] uppercase tracking-[0.28em] text-amber-800 transition hover:bg-amber-100 dark:border-sky-900/70 dark:bg-sky-950/30 dark:text-sky-300"
                    >
                      Destination
                    </button>
                    <div className="inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.28em] text-stone-400">
              ...
              <div className="rounded-2xl border border-dashed border-stone-300 px-4 py-3 dark:border-slate-700">
      ...
      {showEmpty ? (
        <div className="rounded-[1.2rem] border border-dashed border-stone-300 px-4 py-4 text-sm leading-6 text-stone-600 dark:border-slate-700 dark:text-slate-300">
