...
export default function LeaderboardOwnerCell({
  ...
  const href = walletAddress ? `/history/${encodeURIComponent(walletAddress)}` : null
  ...
  return (
    <div className="min-w-0">
      <div className="group flex min-w-0 items-center gap-2">
        ...
        {href ? (
          <a
            href={accountUrl(TESTNET_EXPLORER_URL, walletAddress!)}
            target="_blank"
            rel="noreferrer"
      ...
      {secondaryLabel ? (
        <div className="font-data mt-1 text-xs text-slate-500 dark:text-slate-400">
          {secondaryLabel}
        </div>
      ) : null}
