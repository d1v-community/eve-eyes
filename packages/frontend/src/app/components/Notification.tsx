...
const Notification: FC<INotification> = ({ item, onDismiss }) => {
  const isCloseButtonVisible = item.type !== 'loading'
  ...
  return (
    <div
      className={c(
        'pointer-events-auto group relative w-full overflow-hidden rounded-[1.6rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.97),rgba(248,250,252,0.94))] p-4 text-slate-950 shadow-[0_24px_70px_rgba(15,23,42,0.16)] ring-1 ring-white/70 backdrop-blur-xl transition-all duration-300 dark:border-slate-700/80 dark:bg-[linear-gradient(180deg,rgba(2,6,23,0.96),rgba(15,23,42,0.94))] dark:text-slate-50 dark:ring-white/5 dark:shadow-[0_24px_70px_rgba(2,6,23,0.44)]',
        item.visible
    ...
    >
      ...
      <div className="relative flex min-w-0 items-start gap-3">
        <div className="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-[1rem] border border-white/75 bg-white/75 shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] dark:border-white/10 dark:bg-white/5 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
          <Icon
            className={c('h-5 w-5', tone.iconClassName, {
              'animate-spin': item.type === 'loading',
            })}
          />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <div
            className={c(
              'sds-toast-content mt-1 text-sm leading-6 text-slate-700 [overflow-wrap:anywhere] dark:text-slate-200',
              {
        ...
        {isCloseButtonVisible ? (
          <button
            ...
            className="mt-0.5 inline-flex h-9 w-9 shrink-0 self-start items-center justify-center rounded-full border border-slate-200/80 bg-white/80 text-slate-500 transition hover:border-slate-300 hover:text-slate-900 dark:border-slate-700/80 dark:bg-slate-900/70 dark:text-slate-400 dark:hover:border-slate-600 dark:hover:text-slate-100"
      ...
      <div className="relative mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-800/90">
        <div
          ...
          style={item.type === 'loading' ? undefined : { width: '42%' }}
        />
      </div>
    </div>
  )
